...
@Singleton
public class OsrsWildernessMetalDragonsEquipmentPanel extends JPanel {

    private static volatile OsrsWildernessMetalDragonsEquipmentPanel instance;
    ...
    public OsrsWildernessMetalDragonsEquipmentPanel() {
        setLayout(new BorderLayout());
        this.cacheController = DependencyFactory.getInstance().getInjection(OsrsCacheController.class);

        osrsGridBuilder = new OsrsGridBuilder(OsrsPanels.OSRS_WILDERNESS_METAL_DRAGONS_EQUIPMENT_PANEL);

        try {
            Logger.info("Initializing PanelGrid in OsrsWildernessMetalDragonsEquipmentPanel...");
            ...
            Logger.info("PanelGrid added to OsrsWildernessMetalDragonsEquipmentPanel layout successfully.");
        } catch (Exception e) {
            Logger.error("Error initializing PanelGrid in OsrsWildernessMetalDragonsEquipmentPanel: {}", e.getMessage());
            e.printStackTrace();
        }
    }
