...
@Singleton
public class OsrsLoadoutsExplorationEquipmentPanel extends JPanel {

    private static volatile OsrsLoadoutsExplorationEquipmentPanel instance;
    ...
    public OsrsLoadoutsExplorationEquipmentPanel() {
        setLayout(new BorderLayout());
        this.cacheController = DependencyFactory.getInstance().getInjection(OsrsCacheController.class);

        osrsGridBuilder = new OsrsGridBuilder(OsrsPanels.OSRS_LOADOUTS_MELEE_EQUIPMENT_PANEL);

        try {
            Logger.info("Initializing PanelGrid in OsrsLoadoutsMeleeEquipmentPanel...");
            ...
            osrsGridBuilder.addSeparator("Exploration Equipment Loadout");
            ...
            Logger.info("PanelGrid added to OsrsLoadoutsMeleeEquipmentPanel layout successfully.");
        } catch (Exception e) {
            Logger.error("Error initializing PanelGrid in OsrsLoadoutsMeleeEquipmentPanel: {}", e.getMessage());
            e.printStackTrace();
        }
    }
