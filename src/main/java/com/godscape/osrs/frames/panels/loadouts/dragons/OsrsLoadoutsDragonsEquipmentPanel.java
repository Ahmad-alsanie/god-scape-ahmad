...
@Singleton
public class OsrsLoadoutsDragonsEquipmentPanel extends JPanel {

    private static volatile OsrsLoadoutsDragonsEquipmentPanel instance;
    ...
    public OsrsLoadoutsDragonsEquipmentPanel() {
        ...
        osrsGridBuilder = new OsrsGridBuilder(OsrsPanels.OSRS_LOADOUTS_DRAGONS_EQUIPMENT_PANEL);

        try {
            Logger.info("Attempting to initialize PanelGrid within OsrsLoadoutsDragonsMeleeEquipmentPanel...");
            ...
            Logger.info("PanelGrid added to OsrsLoadoutsDragonsMeleeEquipmentPanel layout successfully.");
        } catch (Exception e) {
            Logger.error("Error initializing PanelGrid in OsrsLoadoutsDragonsMeleeEquipmentPanel: {}", e.getMessage());
            e.printStackTrace();
        }
    }
