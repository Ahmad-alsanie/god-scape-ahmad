...
@Singleton
public class OsrsLoadoutsMetalDragonsEquipmentPanel extends JPanel {

    private static volatile OsrsLoadoutsMetalDragonsEquipmentPanel instance;
    ...
    public OsrsLoadoutsMetalDragonsEquipmentPanel() {
        setLayout(new BorderLayout());
        this.cacheController = DependencyFactory.getInstance().getInjection(OsrsCacheController.class);

        osrsGridBuilder = new OsrsGridBuilder(OsrsPanels.OSRS_LOADOUTS_METAL_DRAGONS_EQUIPMENT_PANEL);

        try {
            Logger.info("Initializing PanelGrid in OsrsLoadoutsMetalDragonsEquipmentPanel...");
            ...
            osrsGridBuilder.addSeparator("Metal Dragons Equipment Loadout");
            ...
            Logger.info("PanelGrid added to OsrsLoadoutsMetalDragonsEquipmentPanel layout successfully.");
        } catch (Exception e) {
            Logger.error("Error initializing PanelGrid in OsrsLoadoutsMetalDragonsEquipmentPanel: {}", e.getMessage());
            e.printStackTrace();
        }
    }
