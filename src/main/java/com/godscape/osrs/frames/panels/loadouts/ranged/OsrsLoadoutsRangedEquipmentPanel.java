...
@Singleton
public class OsrsLoadoutsRangedEquipmentPanel extends JPanel {

    private static volatile OsrsLoadoutsRangedEquipmentPanel instance;
    ...
    public OsrsLoadoutsRangedEquipmentPanel() {
        setLayout(new BorderLayout());
        this.cacheController = DependencyFactory.getInstance().getInjection(OsrsCacheController.class);

        osrsGridBuilder = new OsrsGridBuilder(OsrsPanels.OSRS_LOADOUTS_RANGED_EQUIPMENT_PANEL);

        try {
            Logger.info("Initializing PanelGrid in OsrsLoadoutsRangedEquipmentPanel...");
            ...
            osrsGridBuilder.addSeparator("Ranged Equipment Loadout");
            ...
            Logger.info("PanelGrid added to OsrsLoadoutsRangedEquipmentPanel layout successfully.");
        } catch (Exception e) {
            Logger.error("Error initializing PanelGrid in OsrsLoadoutsRangedEquipmentPanel: {}", e.getMessage());
            e.printStackTrace();
        }
    }
